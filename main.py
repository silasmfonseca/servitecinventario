...
def main(page: ft.Page):
    ...
    def carregar_credenciais():
        ...
        if email and password:
            email_input.value = email; password_input.value = password
            lembrar_me_checkbox.value = True; page.update()

    def apagar_credenciais():
        page.client_storage.remove("auth.email"); page.client_storage.remove("auth.password")

    bg_image = ft.Image(src="banner_servitec.png", fit=ft.ImageFit.COVER, expand=True)
    itens_selecionados = []
    
    header_row = ft.Row(spacing=0)
    header = ft.Container(content=header_row, bgcolor="#f8f9fa", height=40)
    body_list = ft.ListView(expand=True, spacing=0)

    def fechar_dialog(dialog_instance):
        dialog_instance.open = False; page.update()
    ...
    def selecionar_item(item_data, checkbox_control):
        nonlocal itens_selecionados; item_info = {"data": item_data}
    ...
    def abrir_popup_filtro(coluna_db, coluna_label):
        ...
        def aplicar_e_fechar(valor):
            if valor is None or str(valor).strip() == "":
                if coluna_db in filtros_ativos: filtros_ativos.pop(coluna_db, None)
            else: filtros_ativos[coluna_db] = str(valor).strip()
        ...
        if coluna_db in DROPDOWN_OPTIONS:
            ...
            dd.on_change = lambda e: aplicar_e_fechar(dd.value)
            dlg.content = dd
        else:
            tf = ft.TextField(width=300, value=str(valor_atual) if valor_atual else "", hint_text="Digite para filtrar")
            tf.on_submit = lambda e: aplicar_e_fechar(tf.value)
            dlg.content = tf

        def limpar_filtro_coluna_e_fechar(e):
            if coluna_db in filtros_ativos: filtros_ativos.pop(coluna_db, None)
            salvar_filtros_em_storage(); fechar_dialog(dlg)
            atualizar_header_visual(); carregar_dados()

        dlg.actions = [ft.ElevatedButton("Limpar Filtro", on_click=limpar_filtro_coluna_e_fechar), ft.TextButton("Fechar", on_click=lambda e: fechar_dialog(dlg))]
        exibir_dialog(dlg)

    def atualizar_header_visual():
        header_row.controls.clear()
        header_row.controls.append(ft.Container(width=COLUMN_WIDTHS["checkbox"]))
        for col in COLUNAS:
            ...
            header_row.controls.append(
                ft.Container(
                    content=ft.Row([ft.Text(label, weight=ft.FontWeight.BOLD, color=color), ft.Icon(name=ft.icons.FILTER_ALT_ROUNDED, color=color if ativo else ft.colors.TRANSPARENT, size=14)]),
    ...
    def limpar_todos_filtros(e):
        filtros_ativos.clear(); salvar_filtros_em_storage()
        atualizar_header_visual(); carregar_dados()
